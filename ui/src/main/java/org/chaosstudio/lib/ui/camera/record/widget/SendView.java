...
import org.chaosstudio.lib.ui.camera.record.Utils;
...
public class SendView extends RelativeLayout {
    ...
    private void init(Context context){
        width = Utils.getInstance(context).getWidthPixels();
        LayoutParams params = new LayoutParams(width,Utils.getInstance(context).dp2px(180f));
    ...
    public void startAnim(){
        ...
        set.playTogether(
                ObjectAnimator.ofFloat(backLayout,"translationX",0,- width / 3),
                ObjectAnimator.ofFloat(selectLayout,"translationX",0,width / 3)
        );
