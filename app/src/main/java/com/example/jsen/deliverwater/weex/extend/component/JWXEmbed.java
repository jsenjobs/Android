package com.example.jsen.deliverwater.weex.extend.component;
...
public class JWXEmbed extends WXDiv implements WXSDKInstance.OnInstanceVisibleListener,NestedContainer {
    ...
    private JWXEmbed.EmbedRenderListener mListener;
    private HashMap mConfigMap = new HashMap<String, Object>();

    public interface EmbedManager {
        JWXEmbed getEmbed(String itemId);
        void putEmbed(String itemId,JWXEmbed comp);
    }

    public static class FailToH5Listener extends JWXEmbed.ClickToReloadListener {
        @SuppressLint("SetJavaScriptEnabled")
        @Override
        public void onException(NestedContainer comp, String errCode, String msg) {
            //downgrade embed
            if( errCode != null && comp instanceof JWXEmbed && errCode.startsWith("1|")) {
                ...
                webView.loadUrl(((JWXEmbed) comp).src);
            }else{
    ...
    public static class ClickToReloadListener implements OnNestedInstanceEventListener {
        ...
        public void onException(NestedContainer container, String errCode, String msg) {
            if (TextUtils.equals(errCode, WXRenderErrorCode.DegradPassivityCode.
                    WX_DEGRAD_ERR_NETWORK_BUNDLE_DOWNLOAD_FAILED.getDegradErrorCode()) && container instanceof JWXEmbed) {
                final JWXEmbed comp = ((JWXEmbed)container);
    ...
    static class EmbedRenderListener implements IWXRenderListener {
        ...
        EmbedRenderListener(JWXEmbed comp) {
            mComponent = comp;
            mEventListener = new JWXEmbed.ClickToReloadListener();
        }
    ...
    @Deprecated
    public JWXEmbed(WXSDKInstance instance, WXDomObject dom, WXVContainer parent, String instanceId, boolean isLazy) {
        this(instance,dom,parent);
    }

    public JWXEmbed(WXSDKInstance instance, WXDomObject node, WXVContainer parent) {
        super(instance, node, parent);
        mListener = new JWXEmbed.EmbedRenderListener(this);
        ...
        if(instance instanceof JWXEmbed.EmbedManager) {
            Object itemId = node.getAttrs().get(ITEM_ID);
            if (itemId != null) {
                ((JWXEmbed.EmbedManager) instance).putEmbed(itemId.toString(), this);
            }
        }
    }
    ...
    private WXSDKInstance createInstance() {
        ...
        sdkInstance.render("EMBED", WXCacheLocalAssetsUtils.loadAsset(url, sdkInstance.getContext()), mConfigMap, null, WXRenderStrategy.APPEND_ASYNC);
